...
class Model(nn.Module):
  def __init__(self, state_dim, action_dim):
    ...
    self.fc1 = nn.Linear(state_dim + action_dim, 500)
    self.fc2 = nn.Linear(500, 500)
    self.fc3 = nn.Linear(500, state_dim + 1)
...
class MVETD3:
  def __init__(self, state_dim, action_dim, look_ahead=3):
    ...
    self._model = Model(state_dim, action_dim)
    ...
    self._model_optimizer = optim.Adam(self._model.parameters(), lr=0.001)
  ...
  def train_actor_critic(self):
    ...
    if self._global_cycle_counter < 5000:
      return

    targets = torch.cat([batch_next_states - batch_states, batch_rewards], dim=1)
    loss = self._loss_function(self._model([batch_states, batch_actions]), targets.detach())
    self._model_optimizer.zero_grad()
    loss.backward()
    self._model_optimizer.step()

    if self._global_cycle_counter < 10000:
      return

    # Calculate the normal Q-target
    targets = batch_rewards + self._gamma * (1-batch_tfs) * torch.min(*self._critic_target([batch_next_states, batch_next_actions]))
    future_states = batch_next_states
    future_actions = batch_next_actions
    reward_sum = batch_rewards
    current_gamma = self._gamma
    # And do the rollout in the model
    # Add all intermediate results, according to the TD-k trick
    for _ in range(self._look_ahead):
      prediction = self._model([future_states, future_actions])
      future_states += prediction[:, :-1]
      future_rewards = prediction[:, -1].view(-1, 1)
      future_actions = self._actor_target(future_states)
      future_actions += torch.clamp(self._normal.sample(future_actions.size()), -0.5, 0.5).cuda()
      future_actions = torch.clamp(future_actions, -1, 1)
      reward_sum += (current_gamma * future_rewards * (1-batch_tfs))
      current_gamma *= self._gamma
      targets += ((reward_sum + (1-batch_tfs) * current_gamma * torch.min(*self._critic_target([future_states, future_actions]))))

    # And average
    targets /= (self._look_ahead+1)
...
if __name__ == "__main__":
  ...
  look_ahead = 3
  controller = MVETD3(env.observation_space.shape[0], env.action_space.shape[0], look_ahead=look_ahead)
  ...
  while e < global_steps:
    ...
    time_steps.append(e)
    print('[MVE-TD3 -- %s] Step %s: %s' %(args.environment, e, averaged_returns[-1]))

  time_stamp = str(time.time()).replace(".", "")
  np.save("MVE-TD3_%s_%s_returns.npy" %(args.environment, time_stamp), averaged_returns)
  np.save("MVE-TD3_%s_%s_time_steps.npy" %(args.environment, time_stamp), time_steps)